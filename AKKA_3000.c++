...
#include <AFMotor.h> // La siguiente librería describe el movimiento y control de los motores mediante 

AF_DCMotor right(1,MOTOR12_64KHZ); // Frecuencia de PWM (para controlar la velocidad de giro del motor)  64kHz primer motor(recomendado por el módulo de Adafruiten su respectiva datasheet)

AF_DCMotor left(2,MOTOR12_64KHZ); // Hz segundo motor

void setup() {
  
Serial.begin(9600); //La mayoría de ordenadores emplean la variable como 9600, que es lo recomendado por Arduino

pinMode(Trig, OUTPUT); // Se define el Trig como pin de impulso o salida
pinMode(Echo, INPUT); // Se define el Echo como pin de recepción o entrada
...
void loop() { //creo la función loop para la realización de un bucle



digitalWrite(Trig, LOW); // Trigger apagado  
delayMicroseconds(2); //Se utilizó la librería de arduino de delay
  
digitalWrite(Trig, HIGH); // Se enciende trigger 
delayMicroseconds(15); // Se envía un 1 durante 15 microsegundos
...
if (espacio < 20 ) {  // Si la distancia es menor a 20cm girar a la izquierda   right.run(FORWARD);  // Avanza el motor de la derecha (donde BACKWARD es atrás, FORWARD es adelante)
